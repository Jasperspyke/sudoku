...
def print_board(board):
    """Prints the Sudoku board in a readable format with proper spacing and separators."""
    for i in range(9):
        if i % 3 == 0 and i != 0:
            print("_" * 21)  # Print horizontal separator
            print()  # Add a newline after the separator for better spacing
        for j in range(9):
            if j % 3 == 0 and j != 0:
                print("|", end=" ")
            if board[i][j] == 0:
                print(".", end=" ")  # Print . for empty cells
            else:
                print(board[i][j], end=" ")
